...
/-- The cheap counterpart of `unveil`: the same goal shape, without
simplifying the invariant clump.

`unveil` finishes with `veil_simp at *`, which simplifies *every*
hypothesis — and on a module with a large invariant clump that one
hypothesis dominates the cost (measured on a ~3900-cell case study:
~22 s per cell, of which ~14 s is the `at *`). `unveil_local` takes the
route the automatic WP discharger takes — the generated local-WP bridge
theorem — and then simplifies the **goal** only, which leaves it in
exactly the shape `unveil` produces. The clump stays as it is; project
the conjuncts a proof needs with `veil_inv_have`, which normalises them
one at a time (~0.4 s instead of ~22 s on the same cells).

If the cell's VC is not in local-WP form (`veil_apply_local_wp` fails),
fall back to `unveil`. -/
syntax (name := veil_unveil_local) "unveil_local" : tactic

/-- `veil_inv_have h := <invariant>` — bind the conjunct of the invariant
clump that belongs to the named `invariant`/`safety` declaration as `h`,
normalised the way `unveil` would have normalised it.

In a Veil inductiveness goal the induction hypothesis `hinv` is the whole
clump: one right-nested conjunction with a conjunct per declaration, in
declaration order. Written by hand a conjunct is `hinv.2.2.….2.1` with as
many `.2`s as the module has invariants — unreadable, and silently wrong
as soon as a declaration is added or reordered. This builds the
projection chain from the module's own assembled `Invariants` definition
instead, so the index cannot drift from the model.

Two things are checked at elaboration time: the name must be one of the
clump's conjuncts, and the clump in the goal must have exactly as many
conjuncts as the module has invariants — if a model change makes the
pre-simplified clump shape diverge from the declaration list, this fails
loudly rather than projecting the wrong conjunct. -/
syntax (name := veil_inv_have) "veil_inv_have " ident " := " ident : tactic

/-- `veil_solve_frame <invariant>` — the cheap non-SMT rung of the
discharger ladder (`veil.vc.cheapRung`): discharge an
invariant-preservation VC whose action does not write anything the named
invariant reads, without calling a solver.

After `unveil_local` a *frame* cell's goal is already the invariant at the
pre-state behind the action's guards — Veil's own WP simplification has
eliminated the untouched post-state fields — so there is nothing left to
...
def elabVeilFoldBoolAtoms : DesugarTacticM Unit := veilWithMainContext do
  ...
  let targets ← mv.withContext do
    ...
    for decl in (← getLCtx) do
      ...
      -- `whnfR` at each step: a concretized field's type is
      -- `CanonicalField doms Bool`, whose arrow structure is only visible
      -- through the reducible `CanonicalField`/`IteratedArrow`.
      let mut ty ← whnfR (← instantiateMVars decl.type)
      let mut ok := true
      while ty.isForall do
        if ty.bindingBody!.hasLooseBVars then ok := false; break
        ty ← whnfR ty.bindingBody!
  ...
  for decl in targets do
    let (value, «type») ← mv.withContext <| forallTelescopeReducing decl.type fun xs _ => do
...
/-! ## The cheap (non-SMT) frame rung (`veil.vc.cheapRung`) -/
...
invariant abbreviations, and `whnfR` would unfold them away. -/
private partial def invariantAndSpine (e : Expr) : MetaM (Array Expr) :=
  go e #[]
where
...
conjunction `@inv₁ … ∧ @inv₂ … ∧ …`, and which is therefore the
declaration-order authority (the same one `#gen_composition` walks). -/
private def assembledInvariantNames (invariantsName : Name) : MetaM (Array Name) := do
  let some (.defnInfo di) := (← getEnv).find? invariantsName
    | throwError "`{invariantsName}` is not a definition — is the Veil module \
        with the invariant clump in scope?"
  lambdaTelescope di.value fun _ body => do
    (← invariantAndSpine body).mapM fun c => do
      let some n := c.getAppFn.constName?
        | throwError "`{invariantsName}` has a conjunct that is not an \
            application of a named invariant:{indentExpr c}"
      return n

/-- The name under which Veil's WP tactics introduce the invariant clump.
Bound unhygienically by the generated pipeline (see the NOTEs in
`elabSimplifyBeforeConcretizeWp`), so it is looked up by user name. -/
private def veilInvariantClumpHyp : Name := `hinv

@[inherit_doc veil_unveil_local]
def elabVeilUnveilLocal : DesugarTacticM Unit := veilWithMainContext do
...
def elabVeilInvHave (h : Ident) (invIdent : Ident) : DesugarTacticM Unit := veilWithMainContext do
  ...
  let newGoal ← goal.withContext do
    let some hinv := (← getLCtx).findFromUserName? veilInvariantClumpHyp
      | throwError "no `{veilInvariantClumpHyp}` hypothesis in context — \
          `veil_inv_have` expects a goal whose invariant clump the Veil WP \
          pipeline has already introduced (`unveil_local`, `unveil`)"
    let conjuncts ← invariantAndSpine (← instantiateMVars hinv.type)
    unless conjuncts.size == names.size do
      throwError "the invariant clump in the goal has {conjuncts.size} \
        conjuncts but `{invariantsName}` has {names.size} — the clump shape \
        no longer matches the declaration list, so projection by name is not \
        sound here"
    let mut e := mkFVar hinv.fvarId
    for _ in [0:idx] do
      e ← mkAppM ``And.right #[e]
    if idx + 1 < names.size then
      e ← mkAppM ``And.left #[e]
    let (_, goal) ← goal.assertHypotheses #[
      ⟨h.getId, ← inferType e, e, BinderInfo.default, .default⟩
    ]
    return goal
...
Deliberately unhygienic and `__veil`-prefixed: the `exact` that consumes
it is generated in the same place, and an implementation-detail name
...
def elabVeilSolveFrame (invIdent : Ident) : DesugarTacticM Unit := veilWithMainContext do
  ...
  -- while the projected conjunct is always the quantified form. The
  -- ellipsis instantiates them in one elaboration attempt that either
  -- unifies or fails. `apply` would do the same job and was measured at
  -- 3.6 s on a single cell of `VeilTest/Regression/EmbeddingMaxSteps`:
  -- it can succeed leaving subgoals, which `repeat` then re-enters, and
  -- the search fans out. A rung must be cheap even when it loses.
  --
  -- `done` is load-bearing: `repeat` never fails, so without it the rung
...
def elabVeilTactics : Tactic := fun stx => do
  ...
  | `(tactic| unveil_local) => do
    withTraceNode `veil.perf.tactic (fun _ => return "unveil_local") elabVeilUnveilLocal
  | `(tactic| veil_inv_have $h:ident := $inv:ident) => do
