...
private def Module.assembleLabelDef [Monad m] [MonadQuotation m] [MonadExceptOf Exception m] [AddErrorMessageContext m] [MonadOptions m] (mod : Module) : m (Command × Module) := do
...
def Module.assembleLabel [Monad m] [MonadQuotation m] [MonadExceptOf Exception m] [AddErrorMessageContext m] [MonadOptions m] (mod : Module) : m (Array Command × Module) := do
