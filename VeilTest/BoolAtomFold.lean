...
#eval show Lean.Elab.Command.CommandElabM Unit from do
  ...
  -- `allowOpaque := true`: on Lean ≥ 4.32 `value?` returns `none` for a
  -- `theorem` unless opaque values are allowed (see `hasValue`'s same default).
  let some v := info.value? (allowOpaque := true) | throwError "no proof value"
